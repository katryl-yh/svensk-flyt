...
calculated as (
    select
        ...
        case when r.flight_status = 'LAN' then true else false end as is_landed,
        ...
        case 
            when r.actual_arrival_utc is not null and r.scheduled_arrival_utc is not null
            then extract(epoch from (r.actual_arrival_utc - r.scheduled_arrival_utc)) / 60.0
            else null
        end as delay_minutes,
        
        -- Time dimensions
        extract(hour from r.scheduled_arrival_utc) as arrival_hour,
        extract(isodow from r.scheduled_arrival_utc) as arrival_day_of_week,
        strftime(r.scheduled_arrival_utc, '%A') as arrival_day_name,
        date_trunc('day', r.scheduled_arrival_utc) as arrival_date,
        
        -- Time period classification
        case 
            when extract(hour from r.scheduled_arrival_utc) between 6 and 11 then 'Morning (06:00-11:59)'
            when extract(hour from r.scheduled_arrival_utc) between 12 and 16 then 'Midday/Afternoon (12:00-16:59)'
            when extract(hour from r.scheduled_arrival_utc) between 17 and 21 then 'Evening (17:00-21:59)'
            else 'Night/Red-eye (22:00-05:59)'
        end as arrival_time_period,
        
        -- Punctuality flag (on-time = within 15 minutes)
        case 
            when r.actual_arrival_utc is not null 
                and r.scheduled_arrival_utc is not null
                and extract(epoch from (r.actual_arrival_utc - r.scheduled_arrival_utc)) / 60.0 <= 15
            then true
            else false
        end as is_on_time,
        ...
        case 
            when r.first_bag_utc is not null and r.last_bag_utc is not null
            then extract(epoch from (r.last_bag_utc - r.first_bag_utc)) / 60.0
            else null
